package com.staff.canteen.api.observer;
...
public class SynchronizationObserver {
    ...
    public void onSynchronizationUpdate(int type, Object object, int... sort) {
        if (sort.length == 0) {
            for (OnSynchronizationListener listener : map.values()) {
                listener.onSynchronizationUpdate(type, object);
            }
        } else {
            for (int snc : sort) {
                if (map.containsKey(snc)) {
                    map.get(snc).onSynchronizationUpdate(type, object);
                }
            }
        }
    }
